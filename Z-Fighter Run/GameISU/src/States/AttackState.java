 ...
 * Name: Nathan
 * Date: 1/16/2020
 * Purpose: State responsible for the fight between player and brock
 */
package States;
...
public class AttackState extends State{
	...
	private boolean hurt,battle,count,reset=false;
	...
	public AttackState(PocketHandler handler){
		...
		uiManager.addObject(new Button(5,5,252,46,Assets.fireBlast,new ClickListener() {
			...
			public void onCLick() {
				if(attacking) {
				attacking=false;
				oHealth=oHealth-(int)((Math.random()*((120-0)+1))+0);
				}
			}}));
